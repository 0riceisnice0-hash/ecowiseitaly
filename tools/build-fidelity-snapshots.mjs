...
const nativeRoutesFile = path.join(auditRoot, 'native-routes.json');
const nativeRoutes = fs.existsSync(nativeRoutesFile)
  ? JSON.parse(fs.readFileSync(nativeRoutesFile, 'utf8'))
  : [];
const indexedRouteAudit = [...routeAudit.filter((route) => route.inSitemap !== false), ...nativeRoutes.filter((route) => route.inSitemap !== false)];
fs.writeFileSync(path.join(auditRoot, 'indexed-routes.json'), `${JSON.stringify(indexedRouteAudit, null, 2)}\n`, 'utf8');

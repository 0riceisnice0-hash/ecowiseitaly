...
Theme 1.1.4 is deployed to `https://ecowiseitaly.com/`. Production is WordPress 6.8.6 at `/home/customer/www/ecowiseitaly.com/public_html`. The active stylesheet directory is `ecowise-custom-v28`; this is the verified v27 package under a fresh filesystem path so SiteGround PHP workers and browser caches receive the mission-statement update. The directory name is an operational detail only: the theme name, source and release version remain Ecowise Custom 1.1.4.

All legacy plugins remain removed. SiteGround Speed Optimizer is active solely for supported cache management, the front page remains ID 6, the posts page remains ID 2448, the school funnel is page ID 4080, the permalink structure remains `/%postname%/`, and the substantive `wp_` content and production uploads remain in place. The current deployment bundle is `/home/customer/ecowise-deploy-20260807-v27`; it contains the pre-deploy database, active-theme record and administrator inventory. The previous live 1.1.3 release remains at `wp-content/themes/ecowise-custom-v27` for immediate theme-only rollback; the pre-Phase-1 theme remains at `wp-content/themes/ecowise-custom-pre-v23`.

...
The current source release is theme 1.1.1. It retains the approved homepage presentation, adds the code-owned school acquisition funnel, private lead ledger, school-page conversion paths and priority SEO foundation. The funnel now composes its custom PHP content inside the captured Schools shell so its header, hero, logo/navigation, typography and footer match the reconstructed site without an Elementor runtime. The deterministic handoff archive is `ecowise-custom-theme-2026-07-28-v24.zip`: 739 verified theme files, 30,013,931 bytes, SHA-256 `DCBECE27071703AD6F8ADA567C0D0820362F5552E28B0956C4CD5A4F00CEDA4D`. Its machine-readable contract is `release/theme-package.json`. Rebuild future archives with `python3 tools/package-theme.py <output.zip>` and use the checksum printed by the command; do not manually re-zip the directory. The packager normalizes approved text-file line endings and writes a fixed stored-ZIP structure so the same source produces the same archive on Windows and Linux.
...
Theme 1.1.1 is deployed to `https://ecowiseitaly.com/`. Production is WordPress 6.8.6 at `/home/customer/www/ecowiseitaly.com/public_html`. The active stylesheet directory is `ecowise-custom-v25`; this is the verified v24 package under a fresh filesystem path so SiteGround PHP workers compile the complete release. The directory name is an operational detail only: the theme name, source and release version remain Ecowise Custom 1.1.1.

All legacy plugins remain removed. SiteGround Speed Optimizer is active solely for supported cache management, the front page remains ID 6, the posts page remains ID 2448, the school funnel is page ID 4080, the permalink structure remains `/%postname%/`, and the substantive `wp_` content and production uploads remain in place. The current deployment bundle is `/home/customer/ecowise-deploy-20260728-v24`; it contains the pre-deploy database/options/plugin/user records, PHP-lint log and verified v24 archive. The previous live 1.1.0 release remains at `wp-content/themes/ecowise-custom-v24` for immediate theme-only rollback; the pre-Phase-1 theme remains at `wp-content/themes/ecowise-custom-pre-v23`.

...
Theme 1.1.6 is deployed to `https://ecowiseitaly.com/`. Production is WordPress 6.8.6 at `/home/customer/www/ecowiseitaly.com/public_html`. The active stylesheet directory is `ecowise-custom-v30`; the directory name is an operational detail only and the theme remains Ecowise Custom 1.1.6. Theme 1.1.5 remains at `ecowise-custom-v29` and theme 1.1.4 remains at `ecowise-custom-v28` for immediate theme-only rollback.

All legacy plugins remain removed. SiteGround Speed Optimizer is active solely for supported cache management, the front page remains ID 6, the posts page remains ID 2448, the school funnel is page ID 4080, the permalink structure remains `/%postname%/`, and the substantive `wp_` content and production uploads remain in place. The current deployment bundle is `/home/customer/ecowise-deploy-20260823-v29`; it contains the pre-deploy database, active-theme record, plugin inventory, user inventory and PHP-lint record. Earlier rollback themes and the pre-Phase-1 theme remain untouched.

After activation, the production HTTP gate passed all 36 preserved routes, the native school funnel, the exact 36-URL content sitemap, four child sitemaps, four native endpoints, two redirects and HEAD handling. Live browser QA covered the school funnel, Corporate Team Building, Science, Team Building/Wild Rites and Gallery on desktop and mobile: all code-owned assets resolve through `ecowise-custom-v30`, all 33 owner-album files return HTTP 200, the six school-programme headings are present and no checked route has horizontal overflow. Priority titles, descriptions, social metadata and JSON-LD remain present on their configured live routes. WordPress still has exactly one user: administrator `admin` at `marketinghydron@gmail.com`.

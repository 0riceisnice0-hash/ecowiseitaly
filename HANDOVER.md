...
The current source release is theme 1.1.3. It retains the approved homepage presentation, adds the code-owned school acquisition funnel, private lead ledger, school-page conversion paths and priority SEO foundation. The funnel composes its custom PHP content inside the captured Schools shell so its header, hero, logo/navigation, typography and footer match the reconstructed site without an Elementor runtime. Its current offer copy reflects Adam's clarified operating model: EcoWise Italy chooses the programme site and books accommodation/food for the school, the customer pays the separate suppliers, five days/four nights is the recommended international format, and creative/sensory learning is as prominent as science. The deterministic handoff archive is `ecowise-custom-theme-2026-08-06-v26.zip`: 739 verified theme files, 30,017,470 bytes, SHA-256 `7D5516BFB2678CCCC27E4978BB88E339D54D87803993D53F55C3755BF95DBAF8`. Its machine-readable contract is `release/theme-package.json`. Rebuild future archives with `python3 tools/package-theme.py <output.zip>` and use the checksum printed by the command; do not manually re-zip the directory. The packager normalizes approved text-file line endings and writes a fixed stored-ZIP structure so the same source produces the same archive on Windows and Linux.
...
Theme 1.1.3 is deployed to `https://ecowiseitaly.com/`. Production is WordPress 6.8.6 at `/home/customer/www/ecowiseitaly.com/public_html`. The active stylesheet directory is `ecowise-custom-v27`; this is the verified v26 package under a fresh filesystem path so SiteGround PHP workers and browser caches receive the new school-funnel copy and layout. The directory name is an operational detail only: the theme name, source and release version remain Ecowise Custom 1.1.3.

All legacy plugins remain removed. SiteGround Speed Optimizer is active solely for supported cache management, the front page remains ID 6, the posts page remains ID 2448, the school funnel is page ID 4080, the permalink structure remains `/%postname%/`, and the substantive `wp_` content and production uploads remain in place. The current deployment bundle is `/home/customer/ecowise-deploy-20260806-v26`; it contains the pre-deploy database, active-theme record and administrator inventory. The previous live 1.1.2 release remains at `wp-content/themes/ecowise-custom-v26` for immediate theme-only rollback; the pre-Phase-1 theme remains at `wp-content/themes/ecowise-custom-pre-v23`.

After activation, the production HTTP gate passed all 36 preserved routes, the native school funnel, the exact 36-URL content sitemap, four child sitemaps, four native endpoints, two redirects and HEAD handling. The live funnel serves the captured Schools shell once, the 1.1.1 route stylesheet, one H1, one canonical and one schema graph, with no broken images, horizontal overflow or developer-facing document-confirmation language. Priority titles, descriptions, social metadata and JSON-LD remain present on their configured live routes. WordPress still has exactly one user: administrator `admin` at `marketinghydron@gmail.com`.
...
Production now has exactly one WordPress user: ID 1, administrator `admin`, with the sole owner email and email-login address `marketinghydron@gmail.com`. Registration is disabled, all sessions and application passwords have been destroyed, the administrator password was replaced with a newly generated WordPress password and its notification sent only to the owner email, WordPress salts were rotated, and `DISALLOW_FILE_EDIT` prevents dashboard editing of plugin/theme PHP. The restore validator enforces the one-user owner contract whenever `ECOWISE_EXPECTED_ADMIN_EMAIL=marketinghydron@gmail.com` is supplied. The owner should still rotate the SiteGround password and every SSH/SFTP credential through their respective authenticated interfaces; do not store those secrets in Git.

SiteGround accepted the cleanup and restored public web access on 26 July. After reactivation, production returned HTTP 200, the complete deployment validator passed all 36 routes and 35 sitemap URLs, and a fresh browser check confirmed the custom homepage renders correctly. If SiteGround raises another incident, preserve its evidence before changing files and rerun the cleanup/ownership gates before requesting another scan.

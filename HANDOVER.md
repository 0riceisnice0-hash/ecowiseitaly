...
Theme 1.1.9 is deployed to `https://ecowiseitaly.com/`. Production is WordPress 6.8.6 at `/home/customer/www/ecowiseitaly.com/public_html`. The active stylesheet directory is `ecowise-custom-v31`; the directory name is an operational detail only and the theme remains Ecowise Custom 1.1.9. Theme 1.1.6 remains at `ecowise-custom-v30` for immediate theme-only rollback, with earlier versioned releases also retained.

All legacy plugins remain removed. SiteGround Speed Optimizer is active solely for supported cache management, the front page remains ID 6, the posts page remains ID 2448, the school funnel is page ID 4080, the permalink structure remains `/%postname%/`, and the substantive `wp_` content and production uploads remain in place. The current deployment bundle is `/home/customer/ecowise-deploy-20260823-v30`; it contains the pre-deploy database, active-theme record, user inventory and PHP-lint record. Earlier rollback themes and the pre-Phase-1 theme remain untouched.
